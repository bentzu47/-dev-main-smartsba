<template>
  <div :id="`${tagid}_box`" fs-component="account" class="account-group" :data-boxfor="`${tagid}`">
    <div v-if="!showAccLookuptype">
      <div :id="`${tagid}lktype-group`" class="row mt-0 mb-1">
        <div class="col-12">
          <div class="form-check form-check-inline" @click="onClickAccounttype">
            <input class="form-check-input" type="radio" :name="`${tagid}lktype`" :id="`${tagid}lktype-ch1`" value="P" v-model="accounttype" checked/>
            <label class="form-check-label" :label="`${tagid}lktype-ch1_label`" :for="`${tagid}lktype-ch1`">Private Cash</label>
          </div>
          <div class="form-check form-check-inline" @click="onClickAccounttype">
            <input class="form-check-input" type="radio" :name="`${tagid}lktype`" :id="`${tagid}lktype-ch2`" value="W" v-model="accounttype"/>
    ...
    <div :id="`${tagid}field-group`" class="w-100 px-0 form-group">
      ...
      <div class="row m-0">
        <div :class="`form-group px-0 ${accountClass}`">
          ...
          <input v-else-if="mode === 'view'" :name="tagname" :id="tagid" readonly placeholder="N/A" class="form-control read-only" :value="value">
          <small v-if="errorTextFlag.account" :id="`${tagid}HelpText`" :data-for="`${tagid}`" class="form-text text-muted errText">{{ errorText.account }}</small>
        </div>
        <div v-show="showCustname" :class="`form-group pe-0 ${custnameClass}`">
          <input :name="`${custnameId}`" :id="`${custnameId}`" type="text" :for="`${tagid}`" datatype="string" class="form-control-plaintext" tabindex="-1" :value="accountDetail.custname" readonly/>
        </div>
      </div>
    </div>
...
const apiUtil = useRuntimeConfig().public.apiUtil;
const headers = {
  headers: {
...
const queryLookupAccount = async(): Promise<Array<object>> => {
  ...
  await axios.post(`${apiUtil}/lookupaccount/collect`, data, headers)
    .then((result) => {
...
const onClickAccounttype = async() => {
  lookupAcc.value = await queryLookupAccount();
}
