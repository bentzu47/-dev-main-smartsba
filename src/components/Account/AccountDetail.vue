<template>
  <div id="DepositoryInformation">
    <div id="" class="row col-xs-12 col-sm-12 col-lg-12 p-1 pr-2">
      <div class="">
        ...
        <div :class="detailPage.account.className">
          <div class="form-group">
            <AccountLookup 
              ...
              :textLabel="detailPage.account.label" 
              :required="true"
              ...
              @set-account-detail="queryAccountDetail"
              @clear-account-detail="claerAccountDetail" 
            />
          </div>
        </div>
        <div :class="detailPage.depacc.className">
          <div class="form-group">
            <label :id="`${detailPage.depacc.id}_label`" :for="detailPage.depacc.id" class="require">{{ detailPage.depacc.label }}</label>
            <ComboBox 
              v-if="mode === 'insert'"
              :id="detailPage.depacc.id" 
              :name="detailPage.depacc.name"
              :placeholder="detailPage.depacc.label" 
            ...
            />
            <input v-else-if="mode === 'view'" :name="detailPage.depacc.name" :id="detailPage.depacc.id" placeholder="N/A" class="form-control" :value="depInfo.depacc" readonly>
            <!-- <small v-show="errorTextFlag.depacc" :id="`${detailPage.depacc.id}HelpText`" :data-for="detailPage.depacc.id"
              class="form-text text-muted errText">{{ errorText.depacc }}</small> -->
          </div>
        </div>
        <div :class="detailPage.cardid.className">
          <div class="form-group">
            <label :id="`${detailPage.cardid.id}_label`" :for="detailPage.cardid.id">{{ detailPage.cardid.label }}</label>
            <input :name="detailPage.cardid.name" :id="detailPage.cardid.id" placeholder="N/A" class="form-control" :value="depInfo.cardid" readonly>
          </div>
        </div>
      </div>
      <div class="row">
        <div :class="detailPage.prefixcode.className">
          <div class="form-group">
            <label :id="`${detailPage.prefixcode.id}_label`" :for="detailPage.prefixcode.id">{{ detailPage.prefixcode.label }}</label>
            <input :name="detailPage.prefixcode.name" :id="detailPage.prefixcode.id" placeholder="N/A" class="form-control" :value="depInfo.prefixcode" readonly>
          </div>
        </div>
        <div :class="detailPage.prefixdesc.className">
          <div class="form-group">
            <label :id="`${detailPage.prefixdesc.id}_label`" :for="detailPage.prefixdesc.id" class="require">{{ detailPage.prefixdesc.label }}</label>
            <input :name="detailPage.prefixdesc.name" :id="detailPage.prefixdesc.id" placeholder="N/A" class="form-control" :value="depInfo.prefixdesc" :readonly="prefixcodeEditFlag">
        ...
        <div :class="detailPage.custname.className">
          <div class="form-group">
            <label :id="`${detailPage.custname.id}_label`" :for="detailPage.custname.id">{{ detailPage.custname.label }}</label>
            <input :name="detailPage.custname.name" :id="detailPage.custname.id" placeholder="N/A" class="form-control" :value="depInfo.custname" readonly>
          </div>
        </div>
      </div>
...
onMounted(() => {
  if (props.mode === 'view' && props.setValueViewMode) {
...
const queryAccountDetail = (account: string): void => {
  ...
  axios.post(`${apiStock}/inqstkaccdetail/inquiry`, data, headers)
    .then((result) => {
...
const queryLookupDepAcc = (pcflag: string): void => {
  ...
  axios.post(`${apiStock}/lookupdepacct/lookup`, data, headers)
    .then((result) => {
