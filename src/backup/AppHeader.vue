            ...
            <div class="left-col">
                <header :class="`navbar navbar-dark ${!isMenuNavigate && 'shadow-0'}`">
                    <div class="sba-header d-flex">
                        <img class="sba-logo" src="~/assets/images/logo.png" alt="SBA Image Logo">
                ...
                <nav :class="`navbar navbar-dark ${!isMenuNavigate && 'shadow-0'}`">
                    <div class="container-fluid justify-content-end">
                        ...
                        <div v-if="isMenuNavigate" class="position-absolute start-0">
                            <button class="navbar-toggler text-white" type="button" :title="'Hide Navigation'" aria-label="Toggle navigation" @click="$emit('toggle-menu')" >
                                <i class="fa-lg fas fa-bars"></i>
                            </button>
                        </div>
                        <!-- Right elements -->
                        <div>
                            <strong id="usr_fsusername" class="bg-white px-3 py-1 rounded-pill">{{ fsUserName }}</strong>
                        </div>
                    </div>
                </nav>
            </div>
...
const fsUserName = store.getUserName;
...
.navbar-dark {
    /* #333333 -> #404040 */
    background: linear-gradient(90deg, #333333 70%, #404040)
}
</style>